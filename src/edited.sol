...
contract dotMeme_test {
    ...
    constructor() {
        ...
        pomName_testV1 = pomName_test(
            0x6FE0234B5Cae49B27D960f358F0825CA1D6CeC26
        );
    }
    ...
    function countLetters(
        ...
        // 20 character maximum + the leading 1 digit
        require(
        ...
        uint extra = (digitCount - 1) % 3 == 0 ? 0 : 1; //@audit if is dividable by 3, no extra letters, else, consider 1 more letter
        letterCount = (digitCount - 1) / 3;
        letterCount += extra; //@audit get all the letters, so for an 8 lenght character, the lenght would be 3 (3 + 3 + 2)
    ...
    function setPrimaryName(uint256 nameToSet) public {
        // If someone doesn't want a primary name
        if (nameToSet == 0) {
            primaryName[msg.sender] = 0;
        } else {
            checkNameFormat(nameToSet);
            // Check to ensure function caller owns the name
            require(
    ...
    function checkNameFormat(uint256 codedName) private view {
        ...
        for (uint i = 0; i < (digitCountB - 1); i += jump) {
            ...
            charBeingChecked =
                ((codedName / 10 ** i)) -
                ((codedName / 10 ** (i + jump)) * (10 ** jump));

            // this means our letter has 2 decimals digits
            if (charBeingChecked > 122) {
                jump = 2;
                charBeingChecked /= 10; // drop last digit
            }
    ...
    function migrateV1Wallet(
        ...
        for (j = walletLowIndex; j < walletHighIndex; j++) {
            migrateTempName = pomName_testV1.theNames(v1Wallet, j); // Gets the correct name from V1 contract
            if (migrationComplete[migrateTempName] == false) {
                ...
                nameExpiry[migrateTempName] = pomName_testV1.theNameExpiry(
                    migrateTempName
                ); // Migrate expiry time
